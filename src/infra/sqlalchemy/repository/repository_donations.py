...
class RepositoryDonation:
    ...
    def update_receive(self, donation_id: int, receiver_id: int):
        update_receive = update(models.Donations).where(
            models.Donations.id == donation_id).values(receiver_id=receiver_id)
        self.db.execute(update_receive)
        self.db.commit()

    # aqui eu pego id do doador para fazer verificações
    def select_donor_id(self, donor_id: int):
        search = select(models.Donations).where(
            models.Donations.donor_id == donor_id)
    ...
    def get_donations_by_donor(self, donor_id: int):
        donations = self.db.query(models.Donations).where(
            models.Donations.donor_id == donor_id).all()
        return donations
    ...
    def delete_donation(self, donation_id: int):
        delete_stmt = delete(models.Donations).where(
            models.Donations.id == donation_id)
