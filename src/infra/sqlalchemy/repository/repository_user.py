...
class RepositoryUser:
    ...
    def get_id(self,user_id:int):
        search = select(models.Register_user).where(models.Register_user.id  == user_id)
        user = self.db.execute(search).scalar()
        return user

    def delete_user(self, user_id:int):
        delete_stmt = delete(models.Register_user).where(models.Register_user.id == user_id)
        self.db.execute(delete_stmt)
        self.db.commit()   


    def list_user(self,user_id:int):
        user = self.db.query(models.Register_user).where(models.Register_user.id  == user_id)
        return user    
    
    def recover_password(self,user_update:schemas.Recover_password):
        new_password= update(models.Register_user).where(models.Register_user.email == user_update.email).values(password = user_update.password)
