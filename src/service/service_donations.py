...
class DonationService:
    ...
    def service_create(self, donation: Donation, user_logged_in: User):
        if not donation.item_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail='erro,campo obrigatório')
    ...
    def service_receive(self, donation_id: int, usuario_logado: User, background_task: BackgroundTasks):
        ...
        if not donation:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail='Doação não encontrada')
        ...
        if not donor:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail='doador nao encontrado')
        ...
        background_task.add_task(
            self.repository_donation.send_email, donor_info.email, subject, body)

        self.repository_donation.update_receive(
            donation_id, donation.receiver_id)
        return donation
    ...
    def service_get_donations_by_donor(self, donor_id: int, usuario_logado: User):
        ...
        if not donor:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail='Doador não exitente')

        if donor.donor_id != usuario_logado.id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail='Você não possui essas doações')
        donations = self.repository_donation.get_donations_by_donor(
            donor.donor_id)
        return donations
    ...
    def service_edit_donation(self, id: int, donation: Donation_simple, usuario_logado: User):
        ...
        if buscar_id.donor_id != usuario_logado.id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail='Você não possui esse produto')
    ...
    def service_delete(self, donation_id: int, usuario_logado: User):
        ...
        if donation.donor_id != usuario_logado.id:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                                detail='Você não tem autorização para deletar essa doação')

        try:
            self.repository_donation.delete_donation(donation_id)
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Erro ao deletar a doação {e}')
